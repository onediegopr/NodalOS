...
public sealed class CleanClosureGuardTests
{
    [TestMethod]
    public void NoUntrackedCsFilesUnderSrcOrTests()
    {
        var git = FindGit();
        if (git is null)
        {
            Assert.Inconclusive("Git is not available; skipping untracked-source guard.");
            return;
        }
