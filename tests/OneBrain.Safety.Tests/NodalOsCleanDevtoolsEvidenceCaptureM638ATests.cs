...
[TestCategory("M638A")]
public sealed class NodalOsCleanDevtoolsEvidenceCaptureM638ATests
{
    private const string CapturePath = "artifacts/agent-operations/m638a/clean-devtools-evidence-capture.json";
    private const string RuntimePath = "artifacts/agent-operations/m638a/runtime-tab-evidence.json";
    private const string DevtoolsPath = "artifacts/agent-operations/m638a/devtools-console-evidence.json";
    private const string LivenessPath = "artifacts/agent-operations/m638a/bridge-liveness-evidence.json";
    private const string GoNoGoPath = "artifacts/agent-operations/m638a/post-m638a-go-no-go.json";
    private const string ReportPath = "docs/reports/clean-devtools-evidence-capture-m638a.md";
    ...
    [TestMethod]
    public void CaptureIdentifiesM638ABaseMilestoneAndCommit()
    {
        using var doc = ReadJson(CapturePath);
        Assert.AreEqual("M638A", doc.RootElement.GetProperty("milestone").GetString());
        Assert.AreEqual("M638", doc.RootElement.GetProperty("basedOnMilestone").GetString());
        Assert.AreEqual("32fb7c1042b17b3b2061b814db3f1467496645d9", doc.RootElement.GetProperty("basedOnCommit").GetString());
    }
    ...
    {
        using var doc = ReadJson(CapturePath);
        Assert.AreEqual("fail", doc.RootElement.GetProperty("bridgeLivenessEvidence").GetString());
    ...
    [TestMethod]
    public void RuntimeEvidenceRemainsUnknownWhenBridgeLivenessFails()
    {
    ...
    [TestMethod]
    public void LivenessEvidenceRecordsBridgeNotLive()
    {
        using var doc = ReadJson(LivenessPath);
        Assert.IsTrue(doc.RootElement.GetProperty("scriptRun").GetBoolean());
        Assert.AreEqual("fail", doc.RootElement.GetProperty("tcp1270018787").GetString());
        Assert.AreEqual("fail", doc.RootElement.GetProperty("healthEndpoint").GetString());
        Assert.AreEqual("fail", doc.RootElement.GetProperty("runtimeEndpoint").GetString());
        Assert.AreEqual("fail", doc.RootElement.GetProperty("debugEndpoint").GetString());
        Assert.AreEqual("fail", doc.RootElement.GetProperty("configPublicEndpoint").GetString());
        Assert.AreEqual("fail", doc.RootElement.GetProperty("wsUpgradeEndpoint").GetString());
    }
    ...
    [TestMethod]
    public void RecommendedNextMilestoneFollowsEvidenceState()
    {
        using var go = ReadJson(GoNoGoPath);
        using var capture = ReadJson(CapturePath);
        var recommended = go.RootElement.GetProperty("recommendedNextMilestone").GetString();
        Assert.AreEqual("fail", capture.RootElement.GetProperty("bridgeLivenessEvidence").GetString());
        Assert.AreEqual("M638A-Retry Clean DevTools Evidence Capture", recommended);
    }
