...
{
    [TestMethod]
    public async Task MissionDraftRoutePersistsReviewedCandidateAndMissionControlProjectsItSafely()
    {
        RequireWindows();
        ...
        using var request = CreatePost(new Uri(address), token, goal);
        ...
        AssertNoLeak(missionPage, fixture);
    ...
    private static HttpRequestMessage CreatePost(
        ...
        string? origin = null)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, RealWorkspaceMissionDraftEndpointMapper.HtmlRoute)
        {
    ...
    private sealed class WorkspaceFixture : IDisposable
    {
        private WorkspaceFixture(string root)
        {
            ...
            var source = $"var {sensitiveName} = \"{SensitiveFixtureValue}\";";
            File.WriteAllText(Path.Combine(WorkspaceRoot, "src", "Program.cs"), source);
        }
