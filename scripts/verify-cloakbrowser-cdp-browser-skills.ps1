...
function Write-BlockedEvidence {
    ...
    $artifactPath = Join-Path $artifactRoot "cloakbrowser-cdp-browser-skills-$timestamp.redacted.json"
    ...
    $evidence = [ordered]@{
        ...
        captureOk = $false
        domIndexOk = $false
        interactiveElementsDetected = $false
        frictionSignalsDetected = $false
        actionMapOk = $false
...
if ($lock.provider -ne "cloakbrowser" -or $lock.mode -ne "cdp-direct" -or [bool] $lock.system_browser_allowed) {
    Write-BlockedEvidence `
        -Decision "NODAL_OS_CLOAKBROWSER_CDP_BROWSER_SKILLS_PARITY_BLOCKED_WITH_CAUSE" `
...
if ($testExitCode -ne 0) {
    Write-BlockedEvidence `
        -Decision "NODAL_OS_CLOAKBROWSER_CDP_BROWSER_SKILLS_PARITY_BLOCKED_WITH_CAUSE" `
        -Reason "CloakBrowser CDP Browser Skills tests failed with exit code $testExitCode." `
        -RuntimePathInfo $runtimePathInfo `
        -Lock $lock
    exit $testExitCode
}
...
$artifactRoot = Join-Path $RepositoryRoot "artifacts/local-verification"
$latestEvidence = Get-ChildItem -LiteralPath $artifactRoot -Filter "cloakbrowser-cdp-browser-skills-*.redacted.json" |
...
if (-not $latestEvidence) {
    Write-BlockedEvidence `
        -Decision "NODAL_OS_CLOAKBROWSER_CDP_BROWSER_SKILLS_PARITY_BLOCKED_WITH_CAUSE" `
        -Reason "CloakBrowser CDP Browser Skills tests completed but evidence JSON was not found." `
        -RuntimePathInfo $runtimePathInfo `
        -Lock $lock
    exit 4
}
...
"interactiveElementsDetected=$($evidenceJson.interactiveElementsDetected)"
"interactiveElements=$($evidenceJson.domIndexSummary.interactiveElementsCount)"
"frictionSignalsDetected=$($evidenceJson.frictionSignalsDetected)"
"frictionSignals=$($evidenceJson.frictionSignals.Count)"
"actionMapOk=$($evidenceJson.actionMapOk)"
"actionMapEntries=$($evidenceJson.actionMapSummary.entries)"
"screenshotCaptured=$($evidenceJson.screenshotCaptured)"
"externalNavigationBlocked=$($evidenceJson.externalNavigationBlocked)"
"shutdownOk=$($evidenceJson.shutdownOk)"
...
"source=$($evidenceJson.pageSummary.source)"
"runtimeProvider=$($evidenceJson.pageSummary.runtimeProvider)"
"title=$($evidenceJson.pageSummary.title)"
