...
| Chrome / extension / Browser Runtime transition | Move authority away from extension and into Core-governed runtime | Absorbed | M1-M18, M50, M68-M70, M87-M114 | Core-governed Browser Runtime line | Yes | Continue under runtime gates | External CDP general-ready remains blocked | ADRs M1-M18, M50, M68-M70, M91-M114 | High |
...
| M65 | External low-risk auth/live target setup | Closed with target-owned Chrome/CDP/DOM read-only scope | Not absorbed by M51 | Dedicated M65 evidence + formal scope lock | Yes, limited | Treat as closed only for lab.nodalos.com.ar target-owned read-only CDP proof | External CDP general-ready still blocked | ADR M100-M114; ledger ref `audit-ledger-edb3e2fbb0a0446788dae17a269c0058` | High |
...
| External Chrome/CDP/DOM proof | Prove browser runtime against external target | Closed only for target-owned `lab.nodalos.com.ar` read-only proof | M103-M114 | Target-owned proof with isolated profile and ledger | Yes, limited | Keep scope lock; do not claim general external CDP | Third-party/sensitive/production CDP blocked | ADR M109-M114 | High |
...
The next roadmap must not infer that external browser automation is generally ready from M51 HTTP-only proof or from M65 target-owned Chrome/CDP proof.
