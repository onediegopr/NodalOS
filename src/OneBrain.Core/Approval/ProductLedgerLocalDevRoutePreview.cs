...
public sealed class ProductLedgerLocalDevRoutePreview
{
    ...
    public ProductLedgerLocalDevRoutePreviewResult Render(
        ...
        => Render(request, readModelSource, approvalDecisionState, approvedActionExecutionState, boundedApprovedActionState, handoffReportDraftState, workspaceTestJailHandoffDraftState, userWorkspaceAllowlistedHandoffDraftState, latestStateSnapshotState, null);
    ...
    public ProductLedgerLocalDevRoutePreviewResult Render(
        ...
        => Render(request, readModelSource, approvalDecisionState, approvedActionExecutionState, boundedApprovedActionState, handoffReportDraftState, workspaceTestJailHandoffDraftState, userWorkspaceAllowlistedHandoffDraftState, latestStateSnapshotState, latestStateManifestState, durableLatestStateReaderCandidateState, null);
        ...
        ProductLedgerLocalDurableLatestStateAuxiliaryEvidenceResult? durableLatestStateAuxiliaryEvidenceState)
    {
        ...
        if (request is null)
        {
            ...
            return Result(blockers, null, readModelSource, approvalDecisionState, approvedActionExecutionState, boundedApprovedActionState, handoffReportDraftState, workspaceTestJailHandoffDraftState, userWorkspaceAllowlistedHandoffDraftState, latestStateSnapshotState, latestStateManifestState, durableLatestStateReaderCandidateState, durableLatestStateAuxiliaryEvidenceState);
        }
    ...
    private static string ToCanonicalSurfaceHtml(ProductLedgerOperatorSurfaceModel model)
    {
        ...
        html.AppendLine(ToDurableLatestStateAuxiliaryEvidenceHtml(model.DurableLatestStateAuxiliaryEvidenceState));
    ...
    private static string ToLatestStateManifestStateHtml(ProductLedgerLocalOperatorSurfaceLatestStateManifestResult state)
    {
        var html = new StringBuilder();
        html.AppendLine($"    <section data-testid=\"product-ledger-latest-state-manifest-state\" data-state=\"{Encode(state.State.ToString())}\" data-decision=\"{Encode(state.Decision.ToString())}\" data-local-only=\"{Lower(state.LocalOnly)}\" data-internal-only=\"{Lower(state.InternalOnly)}\" data-development-only=\"{Lower(state.DevelopmentOnly)}\" data-historical-index-evidence-only=\"{Lower(state.HistoricalIndexEvidenceOnly)}\" data-authority-live-product=\"{Lower(state.AuthorityLiveProduct)}\" data-product-authority=\"{Lower(state.ProductAuthority)}\" data-create-only=\"{Lower(state.CreateOnly)}\" data-versioned=\"{Lower(state.Versioned)}\" data-json-only=\"{Lower(state.JsonOnly)}\" data-overwrite-allowed=\"{Lower(state.OverwriteAllowed)}\" data-latest-pointer=\"{Lower(state.LatestPointerAvailable)}\" data-latest-pointer-overwrite-allowed=\"{Lower(state.LatestPointerOverwriteAllowed)}\" data-read-precedence=\"{Lower(state.ReadPrecedenceAllowed)}\" data-user-selected-path-allowed=\"{Lower(state.UserSelectedPathAllowed)}\" data-payload-controlled-root-allowed=\"{Lower(state.PayloadControlledRootAllowed)}\" data-canonicalization-passed=\"{Lower(state.CanonicalizationPassed)}\" data-reparse-validation-passed=\"{Lower(state.ReparseValidationPassed)}\" data-production-allowed=\"{Lower(state.ProductionAllowed)}\" data-public-product-allowed=\"{Lower(state.PublicProductAllowed)}\" data-shell-allowed=\"{Lower(state.ShellAllowed)}\" data-command-execution-allowed=\"{Lower(state.CommandExecutionAllowed)}\" data-provider-cloud-network=\"{Lower(state.ProviderCloudNetworkAvailable)}\" data-db-migration=\"{Lower(state.DbMigrationAvailable)}\" data-kms-worm-external-trust=\"{Lower(state.KmsWormExternalTrustAvailable)}\" data-live-automation=\"{Lower(state.BrowserCdpWcuOcrRecipesLiveAvailable)}\" data-pilot-run=\"{Lower(state.PilotRunAvailable)}\" data-release-commercial=\"{Lower(state.ReleaseCommercialReady)}\" data-compliance-custody=\"{Lower(state.ComplianceCustody)}\" data-cloud-backed-durability=\"{Lower(state.CloudBackedDurability)}\">");
        html.AppendLine("      <h2>Latest-state manifest state</h2>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-status\">{Encode(state.StatusText)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-result-kind\">{Encode(state.State.ToString())}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-ids\">manifest={Encode(state.ManifestId)} / action={Encode(state.ActionId)} / source_snapshot={Encode(state.SourceSnapshotId)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-path\">{Encode(state.ManifestRelativePath)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-boundary\">{Encode(state.AllowedBoundary)} {Encode(state.Classification)} json-only immutable versioned create-only no-overwrite no latest pointer no read precedence</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-source-snapshot\">{Encode(state.SourceSnapshotRelativePath)} / snapshot_hash={Encode(state.SourceSnapshotContentHashPrefix)} / checkpoint_hash={Encode(state.SourceSnapshotCheckpointHashPrefix)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-hashes\">manifest_hash={Encode(state.ManifestContentHashPrefix)} / checkpoint_hash={Encode(state.CheckpointHashPrefix)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-latest-state-manifest-stale-policy\">{Encode(state.StalePolicy)}</p>");
        html.AppendLine("      <p data-testid=\"product-ledger-latest-state-manifest-protection\">local/internal/development-only historical index evidence only not authority live/product no public/product path no Production route no broader workspace action no edit update delete no user-selected path no latest pointer no read precedence no shell no subprocess no command execution no Pilot run no live Browser CDP WCU OCR Recipes no network no DB no WORM/KMS no compliance custody no cloud-backed durability no release/commercial no business signoff</p>");
        html.AppendLine("      <div data-testid=\"product-ledger-latest-state-manifest-evidence-refs\">");
        foreach (var evidence in state.EvidenceRefs.OrderBy(evidence => evidence, StringComparer.Ordinal))
        {
            html.AppendLine($"        <p>{Encode(evidence)}</p>");
        }
        ...
        html.AppendLine("      </div>");
        html.AppendLine("      <div data-testid=\"product-ledger-latest-state-manifest-negative-flags\">");
        foreach (var flag in state.NegativeFlags.OrderBy(flag => flag, StringComparer.Ordinal))
        {
            html.AppendLine($"        <p>{Encode(flag)}</p>");
        }
        ...
        html.AppendLine("      </div>");
        html.AppendLine("      <div data-testid=\"product-ledger-latest-state-manifest-blockers\">");
        if (state.Blockers.Count == 0)
        {
            html.AppendLine("        <p>none</p>");
        }
    ...
    private static string ToDurableLatestStateAuxiliaryEvidenceHtml(ProductLedgerLocalDurableLatestStateAuxiliaryEvidenceResult state)
    {
        var html = new StringBuilder();
        html.AppendLine($"    <section data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-state\" data-state=\"{Encode(state.State.ToString())}\" data-decision=\"{Encode(state.Decision.ToString())}\" data-local-only=\"{Lower(state.LocalOnly)}\" data-internal-only=\"{Lower(state.InternalOnly)}\" data-development-only=\"{Lower(state.DevelopmentOnly)}\" data-read-only=\"{Lower(state.ReadOnly)}\" data-auxiliary-evidence-only=\"{Lower(state.AuxiliaryEvidenceOnly)}\" data-authority=\"{Lower(state.Authority)}\" data-live-authority=\"{Lower(state.LiveAuthority)}\" data-product-authority=\"{Lower(state.ProductAuthority)}\" data-read-precedence=\"{Lower(state.ReadPrecedence)}\" data-latest-pointer=\"{Lower(state.LatestPointer)}\" data-latest-pointer-overwrite=\"{Lower(state.LatestPointerOverwrite)}\" data-production-allowed=\"{Lower(state.ProductionAllowed)}\" data-public-product-allowed=\"{Lower(state.PublicProductAllowed)}\" data-shell-allowed=\"{Lower(state.ShellAllowed)}\" data-command-execution-allowed=\"{Lower(state.CommandExecutionAllowed)}\" data-provider-cloud-network=\"{Lower(state.ProviderCloudNetworkAvailable)}\" data-db-migration=\"{Lower(state.DbMigrationAvailable)}\" data-kms-worm-external-trust=\"{Lower(state.KmsWormExternalTrustAvailable)}\" data-live-automation=\"{Lower(state.BrowserCdpWcuOcrRecipesLiveAvailable)}\" data-pilot-run=\"{Lower(state.PilotRunAvailable)}\" data-release-commercial=\"{Lower(state.ReleaseCommercialReady)}\" data-compliance-custody=\"{Lower(state.ComplianceCustody)}\" data-cloud-backed-durability=\"{Lower(state.CloudBackedDurability)}\" data-stale-aware=\"{Lower(state.Validation.StaleAware)}\" data-tamper-detected=\"{Lower(state.Validation.TamperDetected)}\" data-corruption-detected=\"{Lower(state.Validation.CorruptionDetected)}\">");
        html.AppendLine("      <h2>Durable latest-state auxiliary evidence</h2>");
        html.AppendLine($"      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-status\">{Encode(state.StatusText)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-id\">{Encode(state.AuxiliaryEvidenceId)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-classification\">{Encode(state.Classification)} auxiliary evidence only not authority no read precedence no latest pointer stale-aware not public/product Development-only</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-source\">candidate={Encode(state.SourceReaderCandidateId)} / manifest={Encode(state.SourceManifestRelativePath)} / manifest_hash={Encode(state.SourceManifestHashPrefix)} / checkpoint_hash={Encode(state.SourceManifestCheckpointHashPrefix)}</p>");
        html.AppendLine($"      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-validation\">{Encode(state.ValidationState)} / {Encode(state.StaleState)} / {Encode(state.TamperState)} / {Encode(state.CorruptionState)}</p>");
        html.AppendLine("      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-protection\">read-only local/internal/development-only auxiliary evidence only not authority not live authority not product authority no read precedence no latest pointer no latest pointer overwrite no public/product path no Production route no broader workspace action no edit update delete no shell no subprocess no command execution no Pilot run no live Browser CDP WCU OCR Recipes no network no DB no WORM/KMS no compliance custody no cloud-backed durability no release/commercial</p>");
        html.AppendLine("      <div data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-snapshots\">");
        foreach (var snapshot in state.SourceSnapshotRelativePaths.OrderBy(path => path, StringComparer.Ordinal))
        {
            html.AppendLine($"        <p>{Encode(snapshot)}</p>");
        }
        ...
        html.AppendLine("      </div>");
        html.AppendLine("      <div data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-evidence-refs\">");
        foreach (var evidence in state.SourceEvidenceRefs.OrderBy(evidence => evidence, StringComparer.Ordinal))
        {
            html.AppendLine($"        <p>{Encode(evidence)}</p>");
        }
        ...
        html.AppendLine("      </div>");
        html.AppendLine("      <div data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-negative-flags\">");
        foreach (var flag in state.NegativeFlags.OrderBy(flag => flag, StringComparer.Ordinal))
        {
            html.AppendLine($"        <p>{Encode(flag)}</p>");
        }
        ...
        html.AppendLine("      </div>");
        html.AppendLine("      <div data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-blockers\">");
        if (state.Blockers.Count == 0)
        {
            html.AppendLine("        <p>none</p>");
        }
        ...
        html.AppendLine("      </div>");
        html.AppendLine($"      <p data-testid=\"product-ledger-durable-latest-state-auxiliary-evidence-safe-next-step\">{Encode(state.SafeNextStep)}</p>");
        html.AppendLine("    </section>");
        return html.ToString();
    }
