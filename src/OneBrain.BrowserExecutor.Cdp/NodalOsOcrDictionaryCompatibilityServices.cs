...
public sealed class NodalOsOcrDictionaryCompatibilityService
{
    ...
    public IReadOnlyList<NodalOsAlternativeLocalOcrFamilyCandidate> CreateM269AlternativeLocalOcrFamilyCandidates()
    {
        return
        [
            StrategyCandidate(
                "continue-paddleocr-manual-policy",
                "Continue PaddleOCR/RapidOCR and seek manual extra-class policy approval",
                "A",
                "medium",
                "known acquisition path but blocked by class semantics",
                "blocked: PP-OCRv4 and PP-OCRv5 both show dictionary+blank+1",
                "easy in .NET because current ONNX Runtime path already works",
                "medium; depends on external manual policy approval",
                "high unless extra class semantics are approved",
                "low implementation cost, high policy risk",
                "weak until Claude/manual audit approves ignored/reserved extra class",
                "blocked until policy approved",
                requiresManualApproval: true,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.RejectedHypothesisPolicy,
                "ignored-extra-class remains hypothesis-only because PP-OCRv5 class 437 had non-trivial probability"),
            StrategyCandidate(
                "probe-ppocrv5-latin",
                "Probe PP-OCRv5 Latin recognizer/dictionary",
                "B",
                "unknown before probe",
                "official RapidOCR/ModelScope pair exists and is pinnable",
                "unknown until output class count is measured",
                "easy if it uses the same ONNX Runtime path",
                "medium; PP-OCRv5 family may repeat extra-class issue",
                "medium until argmax/probability evidence exists",
                "moderate controlled acquisition/probe cost",
                "good if runtime metadata and dictionary count are captured",
                "possible only if class formula is clean",
                requiresManualApproval: false,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.ViableNeedsProbe,
                "explicit official pair exists but must not be selected until class count/probability probe passes"),
            StrategyCandidate(
                "probe-ppocrv5-chinese-general",
                "Probe PP-OCRv5 Chinese/general recognizer/dictionary",
                "C",
                "unknown before probe",
                "official RapidOCR/ModelScope pair exists and is pinnable",
                "unknown; larger dictionary and broader charset require explicit verification",
                "easy if it uses the same ONNX Runtime path",
                "medium-high due larger model/dictionary surface",
                "medium until class semantics are measured",
                "moderate controlled acquisition/probe cost",
                "good if runtime metadata and dictionary count are captured",
                "possible only if class formula is clean",
                requiresManualApproval: false,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.ViableNeedsProbe,
                "explicit official pair exists but broader charset makes it a second probe behind Latin"),
            StrategyCandidate(
                "review-ppocrv6",
                "Review PP-OCRv6 ONNX+dictionary availability",
                "D",
                "potentially high, but unproven locally",
                "official PaddleOCR docs exist; NODAL OS has no pinned ONNX+dictionary pair yet",
                "unknown until an explicit ONNX+dictionary/config source is identified",
                "unknown; may need conversion or new runtime harness",
                "medium-high while PP-OCRv6 packaging is not pinned",
                "unknown until model/dictionary semantics are audited",
                "higher review/acquisition cost than PP-OCRv5 probes",
                "good if official package provides explicit dict/config",
                "possible after pinned pair and synthetic decode gates",
                requiresManualApproval: false,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.Recommended,
                "best strategic next review because it may avoid the current PP-OCRv4/v5 extra-class dead end without switching away from local OCR"),
            StrategyCandidate(
                "paddle-detector-tesseract-recognizer",
                "Use PaddleOCR detector with Tesseract recognizer fallback",
                "E",
                "variable; stronger on printed text than UI/noisy crops",
                "Tesseract is local and official but not an ONNX CTC recognizer pair",
                "clear for Tesseract traineddata, not compatible with current CTC dictionary gates",
                "moderate; requires process isolation and crop routing instead of ONNX recognizer",
                "medium; introduces native CLI/library dependency",
                "medium; different confidence semantics",
                "moderate integration and sandboxing cost",
                "good if treated as separate fallback with no-authority review",
                "possible only after fallback-specific privacy and confidence gates",
                requiresManualApproval: true,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.FallbackOnly,
                "valid local fallback concept but not a clean primary ONNX recognizer strategy"),
            StrategyCandidate(
                "tesseract-full-fallback",
                "Use Tesseract local as full fallback OCR provider",
                "F",
                "variable",
                "local official OCR engine; no SaaS",
                "not a model+dictionary CTC pair",
                "moderate; requires new executable/library path",
                "medium-high due packaging and language data management",
                "medium; must normalize confidence/no-authority semantics",
                "moderate-high",
                "good if reviewed separately as fallback",
                "possible only after local fallback review",
                requiresManualApproval: true,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.FallbackOnly,
                "should remain fallback review, not primary engine strategy in this block"),
            StrategyCandidate(
                "review-another-onnx-family",
                "Review another local ONNX OCR family",
                "G",
                "unknown",
                "unknown until official lightweight ONNX family is selected",
                "potentially clean if explicit model/dictionary metadata exists",
                "unknown",
                "unknown",
                "unknown",
                "high research cost",
                "good only if official/pinnable sources exist",
                "possible after clean acquisition/runtime gates",
                requiresManualApproval: false,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.ViableNeedsManualReview,
                "reasonable parallel strategy if PP-OCRv6 does not yield a clean pair"),
            StrategyCandidate(
                "keep-ocr-blocked",
                "Keep OCR blocked until a clean pair exists",
                "H",
                "none until engine selected",
                "safest from privacy/authority perspective",
                "not applicable",
                "easy",
                "low maintenance but blocks feature progress",
                "low silent-error risk because no OCR is used",
                "low",
                "strong as safety fallback",
                "none",
                requiresManualApproval: false,
                NodalOsAlternativeLocalOcrFamilyCandidateStatus.Blocked,
                "valid safety fallback if no reviewed local strategy is approved")
        ];
    }

    public NodalOsOcrEngineStrategy CreateM270FinalLocalOcrEngineStrategy()
    {
        var candidates = CreateM269AlternativeLocalOcrFamilyCandidates();
        var recommended = candidates.Single(candidate => candidate.CandidateId == "review-ppocrv6");
        var fallback = new NodalOsOcrFallbackStrategy(
            ...
            ProductiveOcrBlocked: true,
            NoAuthority: true,
            "fallback-only review; not enabled as productive OCR",
            "requires separate sandboxing, packaging, confidence, and no-authority gates");

        return new NodalOsOcrEngineStrategy(
            $"ocr-engine-strategy-{Guid.NewGuid():N}",
            candidates,
            recommended,
            fallback,
            NodalOsAlternativeLocalOcrFamilyDecision.ReadyForPpOcrV6Review,
            CleanModelDictionaryPairPreferred: true,
            ManualApprovalRequiredForIgnoredExtraClass: true,
            ProductiveOcrBlocked: true,
            ShadowModeBlocked: true,
            DecodeBlocked: true,
            NoRawPersistence: true,
            ...
            NoSensitive: true,
            NoSaas: true,
            NoAuthority: candidates.All(candidate => candidate.NoAuthorityCompatible),
            BrowserCredentialRedactor.Redact("Claude-first strategy recommends PP-OCRv6 ONNX+dictionary review before more implementation; PP-OCRv5 ignored-extra-class remains blocked"));
    }

    private static NodalOsAlternativeLocalOcrFamilyCandidate StrategyCandidate(
        string candidateId,
        string name,
        string optionLabel,
        ...
        new(
            ...
            AcquisitionEase: acquisitionEase,
            ModelDictionaryClarity: modelDictionaryClarity,
            DotNetRuntimeEase: dotNetRuntimeEase,
            MaintenanceRisk: maintenanceRisk,
            SilentErrorRisk: silentErrorRisk,
            TechnicalCost: technicalCost,
            EvidenceAuditability: evidenceAuditability,
            FutureShadowModePath: futureShadowModePath,
