...
public sealed class BoundedWorkspacePlanningContextService
{
        ...
        string missionId)
    {
        ...
        var context = new BoundedWorkspacePlanningContext(
            ...
            ExtensionCounts: new Dictionary<string, int>(scan.ExtensionCounts, StringComparer.OrdinalIgnoreCase),
    ...
    public BoundedWorkspacePlanningProjectionResult Build(
        NodalOsBoundedWorkspaceMissionResult missionResult,
        string workspaceId)
    {
        ArgumentNullException.ThrowIfNull(missionResult);
        if (!missionResult.Completed || !missionResult.VerificationPassed)
            return Blocked(["Workspace understanding must complete with verified evidence before planning context is created."]);
        return Build(missionResult.Scan, workspaceId, missionResult.Plan.MissionId);
    }
        ...
        return new NodalOsTaskGraphDraft
        {
            ...
            [
                new NodalOsAssignmentTaskDraft
                {
                    ...
                    AllowedCapabilitiesRedacted = ["filesystem.read evidence review"],
