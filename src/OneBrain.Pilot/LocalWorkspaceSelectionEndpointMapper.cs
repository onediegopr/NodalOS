...
public static class LocalWorkspaceSelectionEndpointMapper
{
    ...
    public static string Render(NodalOsWorkspaceSelectionSnapshot snapshot, string requestToken)
    {
        ...
        var stateClass = snapshot.Accepted
            ? "ready"
            : snapshot.State == NodalOsWorkspaceSelectionState.NotConfigured
                ? "empty"
                : "blocked";
        var title = snapshot.Accepted ? "Workspace local seleccionado" : "Seleccionar workspace local";
        var selectionHeading = snapshot.Accepted ? "Cambiar workspace" : "Elegir workspace";
        ...
        var extensions = snapshot.ExtensionCounts.Count == 0
            ...
            : string.Join(
                string.Empty,
                snapshot.ExtensionCounts
                    .OrderBy(pair => pair.Key, StringComparer.Ordinal)
                    .Select(pair => $"<span class=\"chip\">{H(pair.Key)} · {pair.Value}</span>"));
        ...
        var current = RenderCurrentWorkspace(snapshot);
        var clearForm = snapshot.Persisted ? RenderClearForm(requestToken) : string.Empty;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .nav a { padding: 10px 12px; border-radius: 9px; color: var(--muted); text-decoration: none; border: 1px solid transparent; }
    ...
    .top { display: flex; justify-content: space-between; gap: 18px; align-items: flex-start; border: 1px solid var(--border); background: var(--panel); border-radius: 16px; padding: 22px; }
    .eyebrow { text-transform: uppercase; letter-spacing: .13em; color: #9DB0FF; font-size: 11px; font-weight: 800; }
    h1 { margin: 8px 0 9px; font-size: 32px; letter-spacing: -.035em; }
    .sub { color: var(--muted); line-height: 1.6; max-width: 760px; }
    ...
    .badge.empty { color: #AAB4C0; }
    .badge.blocked { color: var(--warn); border-color: rgba(240,180,90,.45); }
    .grid { display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(300px, .8fr); gap: 18px; margin-top: 18px; }
    ...
    .workspace-name { font-size: 24px; font-weight: 780; margin: 18px 0; }
    .metrics { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .metrics div { border: 1px solid var(--border); background: var(--card); border-radius: 11px; padding: 11px; min-width: 0; }
    .metrics span { display: block; color: var(--muted); font-size: 11px; text-transform: uppercase; letter-spacing: .07em; }
    .metrics strong { display: block; margin-top: 6px; overflow-wrap: anywhere; }
    .chips { display: flex; flex-wrap: wrap; gap: 7px; margin-top: 12px; }
    .chip { display: inline-flex; border: 1px solid var(--border); border-radius: 999px; padding: 5px 8px; color: var(--muted); font-size: 11px; background: #141A21; }
    form { display: grid; gap: 13px; margin-top: 16px; }
    label { font-weight: 700; }
    label span { display: block; color: var(--muted); font-size: 12px; font-weight: 400; margin-top: 4px; line-height: 1.45; }
    input[type=text] { width: 100%; border: 1px solid var(--border); border-radius: 10px; background: #0F141A; color: var(--text); padding: 12px 13px; font: inherit; outline: none; }
    input[type=text]:focus { border-color: var(--blue); box-shadow: 0 0 0 3px rgba(79,124,255,.16); }
    .button { min-height: 40px; border: 1px solid #405891; border-radius: 10px; background: #26375F; color: #EEF2FF; padding: 0 14px; font-weight: 760; cursor: pointer; }
    .button.danger { border-color: rgba(240,106,106,.45); background: rgba(240,106,106,.08); color: #FFB5B5; }
    ...
    ol { padding-left: 22px; color: var(--muted); line-height: 1.55; }
    .limits { color: var(--muted); line-height: 1.55; font-size: 13px; }
    .footer { margin-top: 18px; color: var(--muted); font-size: 12px; display: flex; justify-content: space-between; gap: 12px; }
    .footer a { color: #AABAFF; text-decoration: none; }
...
<body data-nodal-os="workspace-selection" data-selection-state="@@STATE@@" data-persisted="@@PERSISTED@@" data-real-filesystem-read="@@REAL_READ@@" data-workspace-filesystem-mutated="@@WORKSPACE_MUTATED@@" data-secrets-excluded="@@SECRETS_EXCLUDED@@" data-product-authority="@@PRODUCT_AUTHORITY@@">
  <div class="shell">
    <aside class="side">
      <div class="brand">NODAL OS<small>Local workspace boundary</small></div>
      <nav class="nav"><a href="/">Mission Control</a><a class="active" href="/workspace/select">Workspace</a><a href="/workspace/understanding">Understanding</a><a href="/runtime/inspector">Diagnostics</a></nav>
    </aside>
    <main class="main">
      <header class="top">
        <div><div class="eyebrow">Real local workspace · bounded read-only selection</div><h1>@@TITLE@@</h1><div class="sub">@@EXPLANATION@@</div></div>
      ...
      <div class="grid">
        <div>
          ...
          <section class="card" data-section-id="selection-form">
            <div class="card-head"><h2>@@SELECTION_HEADING@@</h2><span class="badge">config local</span></div>
            <form method="post" action="/workspace/select" autocomplete="off">
              <input type="hidden" name="workspaceSelectionToken" value="@@TOKEN@@">
        ...
        <aside>
          <section class="card" data-section-id="review-plan"><div class="card-head"><h2>Plan revisado</h2><span class="badge">@@PLAN_DECISION@@</span></div><ol>@@PLAN@@</ol></section>
...
""";
        ...
        return template
            ...
            .Replace("@@REAL_READ@@", Bool(snapshot.RealFilesystemRead), StringComparison.Ordinal)
            ...
            .Replace("@@SELECTION_HEADING@@", H(selectionHeading), StringComparison.Ordinal)
            .Replace("@@TOKEN@@", H(requestToken), StringComparison.Ordinal)
            .Replace("@@CLEAR_FORM@@", clearForm, StringComparison.Ordinal)
            .Replace("@@PLAN_DECISION@@", H(snapshot.PlanDecision), StringComparison.Ordinal)
            .Replace("@@PLAN@@", plan, StringComparison.Ordinal)
            .Replace("@@BLOCKERS@@", blockers, StringComparison.Ordinal);
    }

    private static string RenderCurrentWorkspace(NodalOsWorkspaceSelectionSnapshot snapshot)
    {
        if (!snapshot.Accepted)
            return string.Empty;
...
<section class="card" data-section-id="current-workspace">
  ...
  <div class="workspace-name">@@DISPLAY_NAME@@</div>
  <div class="metrics">
    <div><span>Workspace ID</span><strong>@@WORKSPACE_ID@@</strong></div>
    <div><span>Root hint</span><strong>@@ROOT_HINT@@</strong></div>
    <div><span>Fingerprint</span><strong>@@FINGERPRINT@@</strong></div>
    <div><span>Path jail</span><strong>@@PATH_JAIL@@</strong></div>
    <div><span>Files read</span><strong>@@FILES_READ@@</strong></div>
    <div><span>Bytes sampled</span><strong>@@BYTES_READ@@</strong></div>
  </div>
...
""";
        ...
        return template
            .Replace("@@DISPLAY_NAME@@", H(snapshot.DisplayNameRedacted), StringComparison.Ordinal)
            .Replace("@@WORKSPACE_ID@@", H(snapshot.WorkspaceId), StringComparison.Ordinal)
            .Replace("@@ROOT_HINT@@", H(snapshot.RootPathHintRedacted), StringComparison.Ordinal)
            .Replace("@@FINGERPRINT@@", H(Short(snapshot.RootPathFingerprint, 16)), StringComparison.Ordinal)
            .Replace("@@PATH_JAIL@@", H(snapshot.PathJailBindingId), StringComparison.Ordinal)
            .Replace("@@FILES_READ@@", snapshot.FilesRead.ToString(CultureInfo.InvariantCulture), StringComparison.Ordinal)
            .Replace("@@BYTES_READ@@", snapshot.TotalBytesRead.ToString(CultureInfo.InvariantCulture), StringComparison.Ordinal)
            .Replace("@@EXTENSIONS@@", extensions, StringComparison.Ordinal);
    }

    private static string RenderClearForm(string requestToken) =>
        $"<form method=\"post\" action=\"{ClearRoute}\" class=\"clear-form\"><input type=\"hidden\" name=\"{TokenField}\" value=\"{H(requestToken)}\"><button class=\"button danger\" type=\"submit\">Quitar selección local</button></form>";

    private static bool IsBoundedFormRequest(HttpRequest request) =>
        request.HasFormContentType &&
        request.ContentLength is > 0 and <= MaximumFormBytes;
