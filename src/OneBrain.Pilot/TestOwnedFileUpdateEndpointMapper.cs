...
public static class TestOwnedFileUpdateEndpointMapper
{
    public const string JsonRoute = "/api/runtime/file-update-fixture";
    public const string HtmlRoute = "/runtime/file-update-fixture";

    public static IEndpointRouteBuilder MapTestOwnedFileUpdateFixture(
        ...
        IHostEnvironment environment)
    {
        ...
        endpoints.MapGet(JsonRoute, async (HttpContext context) =>
        {
            ...
            {
                return Results.NotFound(new { decision = "FILE_UPDATE_FIXTURE_LOCAL_DEV_ONLY" });
            }

            ApplyReadOnlyHeaders(context.Response);
            var result = await new NodalOsTestOwnedFileUpdateMissionScenario()
    ...
    internal static string Render(NodalOsTestOwnedFileUpdateMissionResult result)
    {
        ...
        return $$$"""
            ...
            <body>
              <div class="shell" data-nodal-os="test-owned-file-update" data-local-dev-only="true" data-read-only-surface="true" data-product-authority="false">
                ...
                <main class="main">
                  <section class="grid" aria-label="Exact-hash update state">
                    <article class="card" data-section-id="file-action"><span class="chip">exact-hash update</span><h2>File action</h2><p class="ok">{{{Html(snapshot.FileUpdateState)}}}</p><p class="muted">{{{Html(snapshot.RelativePath)}}} · {{{snapshot.OriginalBytes}}} → {{{snapshot.UpdatedBytes}}} bytes</p></article>
                    <article class="card" data-section-id="verification"><span class="chip">SHA-256</span><h2>Verification</h2><p class="ok">{{{(snapshot.FileUpdateVerified ? "verified" : "failed")}}}</p><code>before {{{Html(snapshot.OriginalSha256 ?? "no-hash")}}}</code><code>after {{{Html(snapshot.UpdatedSha256 ?? "no-hash")}}}</code></article>
                    <article class="card" data-section-id="rollback"><span class="chip">guarded restore</span><h2>Rollback plan</h2><p class="ok">{{{(snapshot.RollbackPlanCreated ? "verified" : "missing")}}}</p><p class="muted">Exact current hash, root fingerprint, target fingerprint and snapshot hash are required.</p></article>
                  </section>
                ...
                <aside class="right" data-section-id="boundaries">
                  ...
                  <div class="metric"><span>Precondition</span><strong>{{{(snapshot.PreconditionMatched ? "matched" : "rejected")}}}</strong></div>
                  <div class="metric"><span>Snapshot</span><strong>{{{(snapshot.SnapshotCreated ? "verified" : "missing")}}}</strong></div>
                  <div class="metric"><span>Atomic replace</span><strong>{{{(snapshot.AtomicReplaceUsed ? "used" : "not used")}}}</strong></div>
                  <div class="metric"><span>General patch</span><strong>disabled</strong></div>
                  <div class="metric"><span>Fixture cleanup</span><strong>{{{(snapshot.TestOwnedFixtureCleaned ? "removed" : "not removed")}}}</strong></div>
                ...
                <footer class="bottom">Local/dev fixture only. Existing target required, exact current SHA-256, verified snapshot, atomic replacement, updated SHA-256, guarded rollback and fixture cleanup. No raw paths, scripts, forms, network, shell, general patch or product authority.</footer>
              </div>
            </body>
            </html>
            """;
