  ...
  <div class="shell">
    <aside class="sidebar">
      ...
      <nav aria-label="Navegación principal">
        ...
        {{Nav("/workspace/select", "Workspace", false)}}
    ...
    <main class="workspace">
      <header class="topbar" data-section-id="topbar">
        <div class="topbar-main"><div class="topbar-label">Misión actual</div><div class="topbar-title">{{H(snapshot.Goal)}}</div><div class="topbar-meta"><span>{{H(snapshot.MissionStatus)}}</span><span>•</span><span>{{snapshot.ProgressPercent}}%</span><span>•</span><span>{{H(snapshot.ActiveModel)}}</span></div></div>
        <div class="actions"><button class="button" type="button" disabled>Pausar</button><button class="button primary" type="button" disabled>{{H(snapshot.ApprovalState)}}</button></div>
      </header>

      <section class="mission-card" data-section-id="mission">
        <div><div class="eyebrow">{{H(snapshot.ProductMode)}} / {{H(snapshot.Decision)}}</div><h1>Una misión, un timeline, control visible.</h1><div class="mission-summary">El runtime, los modelos, los fallbacks, la verificación, el workspace protegido y la evidencia se proyectan aquí sin crear una segunda fuente de verdad.</div></div>
        <div class="mission-metrics">
          <div class="metric"><div class="metric-label">Progreso</div><div class="metric-value">{{snapshot.ProgressPercent}}% · {{H(snapshot.CurrentStep)}}</div><div class="progress-track" aria-label="Progreso de misión"><span class="progress-fill" style="width: {{snapshot.ProgressPercent}}%"></span></div></div>
      ...
      <div class="content-grid">
        <section class="panel" id="timeline" data-section-id="timeline"><div class="panel-header"><h2>Timeline de misión</h2><span>{{snapshot.Timeline.Count}} eventos</span></div><div class="timeline">{{timeline}}</div></section>
        <aside class="panel" data-section-id="context"><div class="panel-header"><h2>Contexto activo</h2><span>runtime</span></div><div class="context-list">{{context}}</div><div class="fallback-note"><strong>Último fallback</strong><br>{{H(fallback)}}</div></aside>
      </div>
      ...
      <div class="lower-grid">
        ...
        <section class="panel" data-section-id="next-step"><div class="panel-header"><h2>Próximo gate productivo</h2><span>P2</span></div><div class="empty">{{H(nextGate)}}<br><a class="workspace-cta" href="/workspace/select">{{H(workspaceAction)}}</a></div></section>
      </div>
