...
{
    public static string Render(NodalOsControlledFixtureVerticalSliceResult result)
    {
        ...
        var actionStatus = result.ControlledAction.Success ? "verified" : "failed closed";
        ...
        return $$$"""
            ...
            <head>
              ...
              <style>
                ...
                *{box-sizing:border-box}body{margin:0;background:#0d1117}.shell{min-height:100vh;display:grid;grid-template-columns:230px minmax(0,1fr) 300px;grid-template-rows:auto 1fr auto}.top{grid-column:1/-1;padding:18px 24px;border-bottom:1px solid #30363d;background:#161b22;display:flex;justify-content:space-between;gap:16px;align-items:center}.top h1{font-size:18px;margin:0}.badge,.chip{display:inline-flex;padding:6px 10px;border:1px solid #4f7cff;border-radius:999px;color:#b8c7ff}.chip{font-size:12px;padding:4px 8px}.side,.right{padding:20px;border-right:1px solid #30363d;background:#161b22}.right{border-right:0;border-left:1px solid #30363d}.main{padding:24px;min-width:0}.card{background:#1c2128;border:1px solid #30363d;border-radius:12px;padding:16px;margin-bottom:16px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.metric{display:flex;justify-content:space-between;gap:16px;padding:8px 0;border-bottom:1px solid #30363d}.metric:last-child{border-bottom:0}.progress{height:8px;background:#30363d;border-radius:999px;overflow:hidden}.progress>span{display:block;height:100%;background:linear-gradient(90deg,#4f7cff,#7c5cff)}ul{padding-left:18px}.timeline{list-style:none;padding:0}.timeline li{position:relative;padding:0 0 18px 24px;border-left:2px solid #30363d}.timeline li:before{content:"";position:absolute;left:-6px;top:4px;width:10px;height:10px;border-radius:50%;background:#4f7cff}.timeline span{color:#aab4c0;font-size:12px}.timeline strong{display:block;margin-top:2px}.timeline p{margin:4px 0;color:#aab4c0}.notice{border-left:3px solid #7c5cff;padding:8px 10px;background:#171b28;color:#d7d0ff}.muted{color:#6e7681}.bottom{grid-column:1/-1;padding:12px 24px;border-top:1px solid #30363d;background:#161b22;color:#aab4c0;font-size:13px}@media(max-width:1000px){.grid{grid-template-columns:1fr}}@media(max-width:900px){.shell{grid-template-columns:1fr}.top,.bottom{grid-column:1}.side,.right{border:0;border-bottom:1px solid #30363d}.right{border-top:1px solid #30363d}}
              </style>
            </head>
            <body>
              <div class="shell" data-nodal-os="runtime-inspector" data-local-dev-only="true" data-read-only="true" data-product-authority="false">
                <header class="top">
                  <div><h1>{{{Html(inspector.Goal)}}}</h1><div class="muted">{{{Html(result.Workspace.DisplayNameRedacted)}}} · {{{Html(inspector.MissionId)}}} · {{{Html(inspector.RunId)}}}</div></div>
                ...
                <main class="main">
                  <section class="grid" aria-label="MVP vertical slice state">
                    <article class="card" data-section-id="approval"><span class="chip">mission scope</span><h2>Approval</h2><p>{{{Html(result.MissionAuthorizationDecision.DecisionKind.ToString())}}}</p><p class="muted">Reused for the authorized read-only capability. No per-step prompt.</p></article>
                    <article class="card" data-section-id="controlled-action"><span class="chip">SafeExecutionFsm</span><h2>Controlled action</h2><p>{{{Html(actionStatus)}}}</p><p class="muted">{{{Html(result.ControlledAction.FinalState.ToString())}}} · read-only fixture · no external IO</p></article>
                    <article class="card" data-section-id="handoff"><span class="chip">verified report</span><h2>Handoff</h2><p>{{{Html(result.Handoff.HandoffPackId)}}}</p><p class="muted">Static, redacted and non-executable.</p></article>
                  </section>
                  ...
                  <section class="card"><h2>Evidence</h2><ul>{{{evidence}}}</ul></section>
                </main>
                <aside class="right" data-section-id="runtime">
                  <h2>Runtime</h2>
                  <div class="metric"><span>Registry</span><strong>{{{Html(result.RegistryEntry.State.ToString())}}}</strong></div>
                  ...
                  <div class="metric"><span>Real filesystem</span><strong>not touched</strong></div>
                  <div class="metric"><span>Network</span><strong>not used</strong></div>
                  <h3>Capabilities</h3><ul>{{{capabilities}}}</ul>
                  <h3>Providers</h3><ul>{{{providers}}}</ul>
                </aside>
                <footer class="bottom">Local/dev read-only Mission Control. No scripts, forms, external resources, customer data, network provider calls or product authority.</footer>
              </div>
            </body>
            </html>
            """;
