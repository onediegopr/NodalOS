...
port.postMessage({ type: 'loadConfig' });

els.connectBtn.addEventListener('click', () => {
  port.postMessage({ type: 'connect', config: currentConfig() });
});
els.disconnectBtn.addEventListener('click', () => port.postMessage({ type: 'disconnect' }));
els.healthBtn.addEventListener('click', () => port.postMessage({ type: 'testHealth', config: currentConfig() }));
els.startBtn.addEventListener('click', () => port.postMessage({ type: 'startRun', instruction: els.instructionInput.value }));
els.pauseBtn.addEventListener('click', () => port.postMessage({ type: 'pause' }));
els.resumeBtn.addEventListener('click', () => port.postMessage({ type: 'resume' }));
els.stopBtn.addEventListener('click', () => port.postMessage({ type: 'stop' }));
els.resumeHumanBtn.addEventListener('click', () => {
  els.humanCard.classList.add('hidden');
  port.postMessage({ type: 'resumeHuman' });
});
